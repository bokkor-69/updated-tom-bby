...
function convertTime(miliSeconds, replaceSeconds = "seconds ", replaceMinutes = "minutes ", replaceHours = "hours ", replaceDays = "days ", replaceMonths = "Months ", replaceYears = "years ", notShowZero = false) {
	if (typeof replaceSeconds == 'boolean') {
		notShowZero = replaceSeconds;
		replaceSeconds = "seconds ";
	}
...
function message(api, event) {
	...
	function applyStyle(text, styleId = "3") {
		const style = styleMap[styleId]?.styledFontMap || styleMap["3"].styledFontMap;
	...
	return {
		...
		style: async (input, styleId = "3", callback) => {
			try {
				global.statusAccountBot = 'good';
				...
				const styledBody = applyStyle(body, styleId);
				...
				if (attachment) {
					messageForm.attachment = attachment;
				}

				return await api.sendMessage(messageForm, event.threadID, callback, event.messageID);
...
stream: async (url, path) => {
	...
	response.data.path = path || `test.${ext}`;
				...
				return await api.sendMessage(messageForm, event.threadID, event.messageID);
			} catch (err) {
				if (JSON.stringify(err).includes('spam')) {
					setErrorUptime();
					throw err;
		...
		edit: async (newContent = "🐸🐸", messageID) => {
			try {
				messageID = messageID || event?.messageReply.messageID || event.messageID;
...
type: async (threadID = event.threadID) => {
			try {
				return await api.sendTypingIndicator(threadID,()=>{}, true);
			} catch (err) {
				if (JSON.stringify(err).includes('spam')) {
					setErrorUptime();
					throw err;
...
fetch: async (url, options = {}, proxy = null, timeout = 10000) => {
    try {
        const headers = {
        ...
        const data = contentType?.includes("application/json")
            ? await response.json()
            : await response.text();
        const res = typeof data === "object" ? JSON.stringify(data, null, 2) : data;
        if (!response.ok) throw new Error(`HTTP Error! Status: ${response.status}\n\n${JSON.stringify(data, null, 2)}`);
        console.log(data);
        return await api.sendMessage(res, event.threadID, event.messageID);
    } catch (err) {
        return await api.sendMessage(`⚠️ Fetch Error: ${err.message}`, event.threadID, event.messageID);
    }
},
		random: async (array) => {
if(!Array.isArray(array)) throw new Error(`The first argument must be a (array)`);
			const randomItem = getRandomItem(array);
			return await api.sendMessage(randomItem, event.threadID, event.messageID);
		},
...
const styleMap = {
   ...
   styledFontMap:{
	...
	"5":{
      ...
      p: 'ᴾ', q: 'ᵟ', r: 'ᴿ', s: 'ˢ', t: 'ᵀ',
      u: 'ᵁ', v: 'ᵛ', w: 'ᵂ', x: 'ˣ', y: 'ᵞ',
      z: 'ᶻ', A: 'ᴬ', B: 'ᴮ', C: 'ᶜ', D: 'ᴰ', E: 'ᴱ',F: 'ᶠ', G: 'ᴳ', H: 'ᴴ', I: 'ᴵ', J: 'ᴶ',K: 'ᴷ', L: 'ᴸ', M: 'ᴹ', N: 'ᴺ', O: 'ᴼ',P: 'ᴾ', Q: 'ᵟ', R: 'ᴿ', S: 'ˢ', T: 'ᵀ',U: 'ᵁ', V: 'ᵛ', W: 'ᵂ', X: 'ˣ', Y: 'ᵞ',Z: 'ᶻ', '0': '𝟬', '1': '𝟭', '2': '𝟮',
      '3': '𝟯', '4': '𝟰', '5': '𝟱', '6': '𝟲', '7': '𝟳',
      '8': '𝟴', '9': '𝟵', '?': '?', '!': '!'
      }
	}
...
function formatMoney(num) {
  const units = ["", "K", "M", "B", "T", "Q", "Qi", "Sx", "Sp", "Oc", "N", "D", "U", "Dd", "Td", "Qd", "Qid", "Sxd", "Spd", "Ocd", "Nd", "V", "Uv", "Ddv", "Tdv", "Qtv", "Qtiv", "Sxv", "Spv", "Ocv", "Nv", "Tg", "Utg", "Dtg", "Ttg", "Qttg", "Qit", "Sxtg", "Sptg", "Octg", "Ntg", "Qg", "Uqg", "Dqg"];
  let unit = 0;
