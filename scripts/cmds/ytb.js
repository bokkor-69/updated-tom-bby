...
async function downloadVideo(url, message) {
  try {
    const response = await axios.get(`https://rest-nyx-apis-production.up.railway.app/api/ytv?d=${encodeURIComponent(url)}&type=mp4`);
...
async function downloadYouTubeAudio(videoId, message) {
  try {
    const response = await axios.get(`https://rest-nyx-apis-production.up.railway.app/api/ytv?d=https://www.youtube.com/watch?v=${videoId}&type=mp3`);
